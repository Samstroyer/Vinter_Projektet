...
class MapManager
{
    ...
    public Tile[,] GenerateMap()
    {
        ...
        Image resourceMap = Raylib.LoadImage(@$"SaveData\Pregens\{selectedImage}");
            ...
            for (int y = 0; y < noiseImage.height; y++)
            {
                ...
                for (int xx = 0; xx < 4; xx++)
                {
                    for (int yy = 0; yy < 4; yy++)
                    {
                        ...
                        mapTiles[xx + x * 4, yy + y * 4] = GenerateTile(brightness, resources);
                    }
                }
            }
        }
    ...
    //sT = stone tile : >200
    private Tile GenerateTile(int light, double richness)
    {
        string alt;
        ...
        return new Tile(alt, light, richness);
    }
}